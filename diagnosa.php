...
switch ($_GET['act']) {
  ...
  default:
    if ($_POST['submit']) {
      ...
      // //tahapan uji coba
      // $arkondisi += array($arkondisi[0] => $arkondisi[1]);
      ...
      $sqlkondisi = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM kondisi order by id");
      while ($rkondisi = mysqli_fetch_array($sqlkondisi)) {
        $arkondisitext[$rkondisi['id']] = $rkondisi['kondisi'];
      }
      
      $sqlpkt = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM penyakit order by kode_penyakit");
...
// --------------------- START ------------------------
      $sqlpenyakit = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM penyakit order by kode_penyakit");
      $arpenyakit = array();
      $rpenyakit = mysqli_fetch_array($sqlpenyakit);
      var_dump($rpenyakit);
      while ($rpenyakit = mysqli_fetch_array($sqlpenyakit)) {
        ...
        $cftotal_temp = 0;
        $cf = 0;
        $sqlgejala = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM basis_pengetahuan where kode_penyakit=$rpenyakit[kode_penyakit]");
        $cflama = 0;
        $rgejala = mysqli_fetch_array($sqlgejala);
      ...
      mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO hasil(
                  tanggal,
                  gejala,
                  penyakit,
                  hasil_id,
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      foreach ($argejala as $key => $value) {
        ...
        $ig++;
        $gejala = $key;
        $sql4 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM gejala where kode_gejala = '$key'");
	 ...
	 <div class='alert alert-success alert-dismissible'>
      ...
      $sql3 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM gejala order by kode_gejala");
      $i = 0;
      while ($r3 = mysqli_fetch_array($sql3)) {
        ...
        $q = mysqli_query($GLOBALS["___mysqli_ston"], $s) or die($s);
